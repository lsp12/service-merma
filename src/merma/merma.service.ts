...
export class MermaService {
  ...
  async findOne(findMermaDto: IFindMerma) {
    ...
    const mermaHaciendad = merma.map((merma) => {
      ...
      const manosMerma = merma.DesgloceManos.map((desgloce) => {
        ...
        const porcentaje = peso / mermaCortada;
      ...
      //Agrupamos caja propia y de terceros
      ...
      return {
        ...
        porcentajeContado: Number(porcentajeContado.toFixed(2)),
        porcentajeProcesado: Number(porcentajeProcesado.toFixed(2)),
  ...
  async findByWeek(week: number) {
    const weekNow = moment().format('YYYY-MM-DD');
    const date = moment().isoWeek(week).format('YYYY-MM-DD');
    const dateEnd = moment().isoWeek(week).add(6, 'days').format('YYYY-MM-DD');
    const merma = await this.MermaRepository.find({
      where: {
        fecha: Between(date, dateEnd),
        mermaCortada: MoreThan(0),
      },
      relations: [
        'ranch',
    ...
    const mermaHaciendad = merma.map((merma) => {
      ...
      defectosMerma.map((defecto, index) => {
        const porcentajeMM = (defecto.pesoRacimo / pesoMermaEv) * 100;
        aux[index] = {
          porcentajeMM,
          defecto: defecto.defecto,
    ...
    const promedioMerma =
      mermaHaciendad.reduce((a, b) => {
        return a + Number(b.porcentajeContado);
      }, 0) / mermaHaciendad.length;

    return { promedioMerma, count: mermaHaciendad.length, mermaHaciendad };
  }

  async getPerfilesRacimos(ranch: number, week: number) {
    ...
    const merma = await this.MermaRepository.find({
      ...
      relations: [
        ...
        'perfilRacimos.pesoMano',
        'perfilRacimos.numeroDedos',
        'perfilRacimos.calibraciones',
        'perfilRacimos.longitudDedos',
      ],
      order: {
        fecha: 'ASC',
      },
